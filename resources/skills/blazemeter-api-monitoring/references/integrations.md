...
3. Copy the callback URL and paste it into the webhook notifications section of your BlazeMeter API Monitoring [test environment](skill-blazemeter-api-monitoring://references/configuration.md#api-monitoring-managing-configuration-con-environments) or [shared environment](skill-blazemeter-api-monitoring://references/configuration.md#api-monitoring-managing-configuration-con-environments) settings.
...
If you want to customize the code and add more information to the incident, go to [BlazeMeter API Monitoring webhook payload](skill-blazemeter-api-monitoring://references/notifications.md#api-monitoring-advanced-webhooks) to see what properties are available.
...
Assertions can be created on the data generated by the Ghost Inspector test step. A JSON object containing data from the test run can be evaluated using the [JSON syntax used in assertions for HTTP request steps](skill-blazemeter-api-monitoring://references/scripting.md#validating-responses-with-assertions). An assertion for the status of the UI test run is created by default.

### Creating Variables

Data from the test run can be extracted and stored in a [variable](skill-blazemeter-api-monitoring://references/advanced-features.md#dynamic-data-and-request-chaining) for use in subsequent requests. The same syntax for extracting data from HTTP request steps can be used for extracting data from the Ghost Inspector test run JSON result.
...
After parsing the data, you can access it to create script [assertions](skill-blazemeter-api-monitoring://references/scripting.md#script-engine-overview) or [variables](skill-blazemeter-api-monitoring://references/scripting.md#script-engine-overview) using the same code as you use for scripts attached to HTTP request steps.
...
The next steps would be to create additional [assertions](skill-blazemeter-api-monitoring://references/scripting.md#validating-responses-with-assertions) for each test to validate their data, and set them on a [schedule](skill-blazemeter-api-monitoring://references/configuration.md#api-monitoring-scheduling-test-runs).
