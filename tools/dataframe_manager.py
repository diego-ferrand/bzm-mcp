...
def _serialize_record(record: "DataFrameRecord") -> Dict[str, Any]:
    payload = record.to_metadata(include_schema=True)
    payload["data"] = record.dataframe.to_dicts()
    return payload


def _deserialize_record(payload: Dict[str, Any]) -> "DataFrameRecord":
    rows = payload.get("data") or []
    dataframe = pl.DataFrame(rows) if rows else pl.DataFrame()
    schema_rows = payload.get("schema") or _to_schema_rows(dataframe)
    return DataFrameRecord(
        dataframe_id=str(payload["dataframe_id"]),
        table_name=str(payload.get("table_name") or f"df_{payload['dataframe_id']}"),
        created_at=str(payload.get("created_at") or datetime.now(UTC).isoformat()),
        origin_manager=str(payload.get("origin_manager") or ""),
        origin_action=str(payload.get("origin_action") or ""),
        rows=int(payload.get("rows") if payload.get("rows") is not None else dataframe.height),
        columns=int(payload.get("columns") if payload.get("columns") is not None else dataframe.width),
