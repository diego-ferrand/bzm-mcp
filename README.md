...
## Publishing to the Docker catalog (Docker Hub)

CI builds multi-platform images and publishes to GitHub Container Registry (GHCR). Images are also published to **Docker Hub** (the Docker catalog) when enabled.

**Before every publish**, [Docker Scout](https://docs.docker.com/scout/) runs on the image. The job **fails** if critical or high vulnerabilities are found, so only images that pass the Scout review are pushed.

**To publish to Docker Hub:**

1. In the repo: **Settings → Secrets and variables → Actions**
2. Add secrets: `DOCKERHUB_USERNAME` (Docker Hub username), `DOCKERHUB_TOKEN` (access token or PAT).
3. Add variable: `PUBLISH_TO_DOCKERHUB` = `true`.
4. Optionally set `DOCKERHUB_IMAGE_NAME` (e.g. `blazemeter/bzm-mcp`; default is `blazemeter/bzm-mcp`).

The same image and tags (e.g. `latest`, semver) are pushed to GHCR and, when enabled, to Docker Hub.

**Run Scout locally** (same checks as CI, before pushing):

1. **Prereqs:** [Docker](https://docs.docker.com/get-docker/) with the Scout plugin (included in Docker Desktop 4.17+), a built Linux binary in `dist/` (e.g. from `uv run build.py` or from a release), and **logged in to Docker Hub** so Scout can use the vulnerability database:
  ```bash
   docker login
  ```
   (Or sign in via Docker Desktop.)
2. Build the image and run Scout:
  ```bash
   ./scripts/scout-local.sh
  ```
   Or with a custom tag: `./scripts/scout-local.sh myimage:tag`
3. To fail when critical/high CVEs are found (same as CI): `SCOUT_EXIT_CODE=1 ./scripts/scout-local.sh`
  **Note:** Scout may show “Base image was auto-detected” for local builds. CI builds use provenance attestations when pushing, so pipeline results are more accurate.

**Publish to Docker Hub manually (from your machine):**

1. **Prerequisites**
  - Docker installed and running.
  - A [Docker Hub](https://hub.docker.com) account.
  - Linux binaries in `dist/` (e.g. run `uv sync && uv run build.py`, or use binaries from a release).
2. **Create the repository on Docker Hub** (if it doesn't exist)
  - Go to [hub.docker.com](https://hub.docker.com) → **Repositories** → **Create Repository**.
  - Choose a name (e.g. `bzm-mcp`) and optionally a namespace (e.g. `blazemeter`). The full image name will be `<namespace>/<name>` (e.g. `blazemeter/bzm-mcp`).
3. **Log in to Docker Hub**
  ```bash
   docker login
  ```
   Use your Docker ID and password or a [Personal Access Token](https://docs.docker.com/go/access-tokens/).
4. **Build the image with your Docker Hub tag**
  From the repo root (replace `YOUR_DOCKERHUB_USERNAME` with your Docker Hub username or org, e.g. `blazemeter`):
   For a versioned tag as well:
5. **Review with Docker Scout (recommended)**
  ```bash
   docker scout quickview YOUR_DOCKERHUB_USERNAME/bzm-mcp:latest
  ```
   Fix or accept any findings before publishing.
6. **Push to the Docker catalog**
  ```bash
   docker push YOUR_DOCKERHUB_USERNAME/bzm-mcp:latest
   docker push YOUR_DOCKERHUB_USERNAME/bzm-mcp:v1.0.0   # if you created a version tag
  ```
7. **Optional: multi-platform (linux/amd64 + linux/arm64)**
  You need both binaries in `dist/` (e.g. from CI artifacts). Then:

Your image will appear at `https://hub.docker.com/r/YOUR_DOCKERHUB_USERNAME/bzm-mcp`.
